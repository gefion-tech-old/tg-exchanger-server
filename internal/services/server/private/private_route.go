...
func (pr *PrivateRoutes) ConfigurePrivateRouter(router *gin.RouterGroup, g guard.GuardI) {
...

	{
		...
		admin.PUT("/message/:id", g.AuthTokenValidation(), g.IsAuth(), pr.updateBotMessageHandler)
		admin.DELETE("/message/:id", g.AuthTokenValidation(), g.IsAuth(), pr.deleteBotMessageHandler)
